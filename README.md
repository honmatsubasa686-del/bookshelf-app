...
`./vendor/bin/sail npm run dev` は起動したままにして、別ターミナルでブラウザ確認や Artisan コマンドを実行してください。

`Vite manifest not found at: public/build/manifest.json` が表示された場合は、`./vendor/bin/sail npm run dev` が起動しているか確認してください。

Mac側の npm と Sail側の npm を混在させると、Rollup の optional dependency が不足して Vite が起動しない場合があります。
その場合は、以下を実行してください。
