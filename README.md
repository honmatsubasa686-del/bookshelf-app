...
`npm run dev` は起動したままにして、別ターミナルでブラウザ確認や Artisan コマンドを実行してください。

`Vite manifest not found at: public/build/manifest.json` が表示された場合は、`npm run dev` が起動しているか確認してください。
